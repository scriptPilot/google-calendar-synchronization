...
function runOneWaySync(
  ...
  Calendar.CalendarList.list({ showHidden: true }).items.forEach((cal) => {
    if (
      cal.summaryOverride === sourceCalendarName ||
      cal.summary === sourceCalendarName
    )
      sourceCalendar = cal;
  });
  if (!sourceCalendar)
    throw new Error(`Source calendar ${sourceCalendarName} not found.`);
  ...
  Calendar.CalendarList.list({ showHidden: true }).items.forEach((cal) => {
    if (
      cal.summaryOverride === targetCalendarName ||
      cal.summary === targetCalendarName
    )
      targetCalendar = cal;
  });
  if (!targetCalendar)
    throw new Error(`Target calendar ${targetCalendarName} not found.`);

  // Allow only one waiting script per sync job
  const waitingKey = sourceCalendar.id + ">" + targetCalendar.id + ":waiting";
  const waitingValue =
    PropertiesService.getUserProperties().getProperty(waitingKey);
  if (waitingValue === "yes") {
    console.info(
      "Script call cancelled because another script call is already waiting.",
    );
    return;
  } else {
  ...
  const startDate = new Date(
    todayStart.getFullYear(),
    todayStart.getMonth(),
    todayStart.getDate() - previousDays,
  );
  const endDate = new Date(
    todayEnd.getFullYear(),
    todayEnd.getMonth(),
    todayEnd.getDate() + nextDays,
  );

  // Get last update from properties (if property is empty, last update will be 1970-01-01)
  const lastUpdateGiven = PropertiesService.getUserProperties().getProperty(
  ...
  while (pageToken !== undefined) {
    respectQuota();
    const response = Calendar.Events.list(sourceCalendar.id, {
      pageToken,
  ...
  while (pageToken !== undefined) {
    respectQuota();
    const response = Calendar.Events.list(targetCalendar.id, {
      pageToken,
      singleEvents: true,
      showDeleted: lastUpdateGiven ? true : false,
      updatedMin: lastUpdateGiven ? lastUpdate.toISOString() : null,
    ...
    sourceEvents.forEach((sourceEvent) => {
      if (
        !existingEvents.filter(
          (event) =>
            event.extendedProperties?.private?.sourceEventId === sourceEvent.id,
        ).length
      ) {
        respectQuota();
        existingEvents.push(
          ...Calendar.Events.list(targetCalendar.id, {
            privateExtendedProperty: `sourceCalendarId=${sourceCalendar.id}`,
    ...
    existingEvents.forEach((existingEvent) => {
      if (
        !sourceEvents.filter(
          (event) =>
            event.id ===
            existingEvent.extendedProperties?.private?.sourceEventId,
        ).length
      ) {
        respectQuota();
        const sourceEvent = Calendar.Events.get(
          sourceCalendar.id,
          existingEvent.extendedProperties?.private?.sourceEventId,
        );
  ...
  sourceEvents.forEach((sourceEvent) => {
    ...
    const existingEvent = relevantExistingEvents.length
      ? relevantExistingEvents[0]
      : null;
    ...
    if (!existingEvent) {
      ...
      if (targetEvent.status !== "cancelled") {
        ...
        try {
          ...
          respectQuota();
          const existingEvent = Calendar.Events.insert(
            targetEvent,
            targetCalendar.id,
          );
      ...
      Object.keys(targetEvent).forEach(
        (key) => (harmonizedExistingEvent[key] = existingEvent[key]),
      );
      if (!harmonizedExistingEvent.colorId && targetEvent.colorId === "0")
        harmonizedExistingEvent.colorId = "0";
      const harmonizedExistingEventString = JSON.stringify(
        sortObject(harmonizedExistingEvent),
      );
      ...
      if (targetEventString !== harmonizedExistingEventString) {
        ...
        try {
          ...
          // Log update
          const action =
            targetEvent.status === "cancelled" ? "Deleted" : "Updated";
