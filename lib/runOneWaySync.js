...
function runOneWaySync(sourceCalendarName, targetCalendarName, previousDays, nextDays, correctionFunction) {
  ...
  existingEvents.forEach(existingEvent => {
    ...
    if (sourceEvent) {
      ...
      // Check if the source event is within the date range
      const sourceEventStart = sourceEvent.start.dateTime
        ? new Date(sourceEvent.start.dateTime)
        : new Date(sourceEvent.start.date.substr(0, 4), sourceEvent.start.date.substr(5, 2) - 1, sourceEvent.start.date.substr(8, 2))
      const sourceEventEnd = sourceEvent.end.dateTime
        ? new Date(sourceEvent.end.dateTime)
        : new Date(sourceEvent.end.date.substr(0, 4), sourceEvent.end.date.substr(5, 2) - 1, sourceEvent.end.date.substr(8, 2))
      const withinDateRange = !(sourceEventEnd < startDate || sourceEventStart > endDate)

      // Check if the source event is not deleted
      const notDeleted = sourceEvent.status !== 'cancelled'

      // Add source event to the array if within the date range and not deleted    
      if (withinDateRange && notDeleted) sourceEvents.push(sourceEvent)
