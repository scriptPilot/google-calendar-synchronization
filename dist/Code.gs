...
function onAirbusToDennisUpdate() {
  const daysToLastMonday = (new Date().getDay()) === 0 ? 6: (new Date().getDay()) - 1
  runOneWaySync('AirbusToDennis', 'Termine', daysToLastMonday, 9999, (targetEvent, sourceEvent) => {

    // Keep summary
    if (sourceEvent.summary) targetEvent.summary = sourceEvent.summary

    // Keep location
    if (sourceEvent.location) targetEvent.location = sourceEvent.location

    // Keep color id
    targetEvent.colorId = sourceEvent.colorId
